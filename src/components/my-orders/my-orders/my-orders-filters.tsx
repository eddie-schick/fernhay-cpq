...
import BOMIDFilters from "./filters-UI/bom-id-filter";
...
function MyOrdersFilters() {
      ...
      const filterChipObj = {
        ...
        value:
          filterValues[MY_ORDERS_FILTERS.CUSTOMER_NAME].values.length > 1
            ...
            : filterValues[MY_ORDERS_FILTERS.CUSTOMER_NAME].values[0].title,
      };
      filterList.push(filterChipObj);
    }
  ...
  return (
    <StyledFilterContainer className="my-orders__filter-container">
      <Badge
        badgeContent={
          activeFiltersForChips.length > 1 ? activeFiltersForChips.length : null
        }
        color="primary"
      >
        <Button
        ...
        >
          {!isFilterBtnHover ? <FilterIcon /> : <FilterLogoHoverIcon />}
...
const StyledChipContainer = styled(MuiBox)<{ drawerOpen?: boolean }>(
  ({ theme, drawerOpen = false }) => ({
    ...
    ".chip-container": {
      ...
      borderRadius: "0.5rem",
      background: theme.palette.custom.blueBackground,
      display: "flex",
      flexDirection: "row",
      gap: "0.4rem",
      alignItems: "center",
